...
import type { ChronoTheraApiComponent, ChronoTheraExcipientComponent, ChronoTheraSimulation } from '@/lib/types/chronothera';
...
export function ChronoTheraApp(){
 ...
 const [apis,setApis]=useState<ChronoTheraApiComponent[]>([{name:'Candidate API', dose_mg:10}]); const [excipients,setExcipients]=useState<ChronoTheraExcipientComponent[]>([]); const [selected,setSelected]=useState<ChronoTheraSimulation>();
 useEffect(()=>{ if(!assetId && assets[0]) setAssetId(assets[0].id); },[assets,assetId]);
 useEffect(()=>{ if(asset){ setApis(asset.default_apis.map(name=>({name,dose_mg:10,modality:asset.modality}))); setObjective(asset.formulation_objectives[0] ?? objective); setRoute(asset.suggested_routes[0] ?? route); } },[assetId]);
 const validation=useMemo(()=>{ const total=excipients.reduce((s,e)=>s+Number(e.percentage||0),0); if(!apis.length || apis.some(a=>!a.name || a.dose_mg<=0)) return 'At least one API with positive dose is required.'; if(!excipients.length) return 'Select at least one excipient strategy.'; if(total>100) return 'Excipient percentages cannot exceed 100% total.'; return ''; },[apis,excipients]);
