...
import { useCreateCXU, useZones, useCXUs } from '@/lib/hooks/use-simulations';
...
const DRAFT_KEY = (id: string) => `genovate:draft:cxu:${id}`;
...
export function CreateCXUDialog({ programId }: { programId: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (draft.cxuType) setCxuType(draft.cxuType);
      if (draft.zoneId) setZoneId(draft.zoneId);
    } catch {
  ...
  const saveDraft = useCallback(() => {
    localStorage.setItem(
      DRAFT_KEY(programId),
      JSON.stringify({ formValues: values, cxuType, zoneId }),
    );
    toast.success('Draft saved');
  }, [programId, values, cxuType, zoneId]);

  const onRunNow = handleSubmit(async (v) => {
    if (!cxuType) { toast.error('CXU type is required'); return; }
    if (!zoneId) { toast.error('Zone is required'); return; }
    await createCXU.mutateAsync(
      {
        zone_id: zoneId,
        cxu_type: cxuType,
        configuration: buildConfig(v, matrixPairs),
      ...
      >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="configure" className="space-y-3 mt-2">
            <div>
              <label className="text-sm font-medium" htmlFor="cx-name">
                Name <span className="text-destructive">*</span>
              </label>
              <Input id="cx-name" {...register('name', { required: true })} placeholder="e.g. CXU-001" />
            </div>
            <div>
              <label className="text-sm font-medium">
                CXU type <span className="text-destructive">*</span>
              </label>
              <Select value={cxuType} onValueChange={setCxuType}>
                <SelectTrigger><SelectValue placeholder="Select CXU type" /></SelectTrigger>
                <SelectContent>
                  {CXU_TYPES.map((t) => (
            ...
            <div>
              ...
              <Select value={zoneId} onValueChange={setZoneId}>
                <SelectTrigger>
                  <SelectValue placeholder={zoneList.length ? 'Select zone' : 'No zones — create one first'} />
                </SelectTrigger>
                <SelectContent>
                  {zoneList.map((z) => (
            ...
            <div>
              <label className="text-sm font-medium" htmlFor="cx-obj">Simulation Objective</label>
              <Textarea
                id="cx-obj" rows={2}
            ...
            <div className="rounded-md border">
              ...
              {advancedOpen && (
                <div className="border-t px-3 pb-3 pt-2 space-y-3">
                  ...
                  <div>
                    <label className="text-sm font-medium" htmlFor="cx-guardian">Guardian ID</label>
                    <Input id="cx-guardian" {...register('guardian_id')} placeholder="UUID or guardian name" />
                  </div>
                  <div>
                    <label className="text-sm font-medium" htmlFor="cx-drift">Drift Threshold (0–1)</label>
                    <Input
                      id="cx-drift" type="number" min={0} max={1} step={0.01}
          ...
          <TabsContent value="expert" className="space-y-4 mt-2">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <div>
                  <label className="text-sm font-medium" htmlFor="cx-esc">Escalation Threshold (0–1)</label>
                  <Input
                    id="cx-esc" type="number" min={0} max={1} step={0.01}
                    placeholder="e.g. 0.8"
        ...
        <DialogFooter className="gap-2 pt-2">
          ...
          <Button
            type="button"
            onClick={onRunNow}
            disabled={!cxuType || !zoneId || createCXU.isPending}
          >
            {createCXU.isPending ? 'Creating…' : 'Run Now →'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
