...
import { useCreateZone, useZones, useCXUs } from '@/lib/hooks/use-simulations';
...
const DRAFT_KEY = (id: string) => `genovate:draft:zone:${id}`;
...
export function CreateZoneDialog({ programId }: { programId: string }) {
  ...
  const buildConfig = useCallback(
    (v: FormValues, cid: string, pairs: typeof matrixPairs) =>
      stripEmpty({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (draft.zoneType) setZoneType(draft.zoneType);
      if (draft.candidateId) setCandidateId(draft.candidateId);
    } catch {
  ...
  const saveDraft = useCallback(() => {
    localStorage.setItem(
      DRAFT_KEY(programId),
      JSON.stringify({ formValues: values, zoneType, candidateId }),
    );
    toast.success('Draft saved');
  }, [programId, values, zoneType, candidateId]);

  const onRunNow = handleSubmit(async (v) => {
    if (!zoneType) { toast.error('Zone type is required'); return; }
    await createZone.mutateAsync(
      { program_id: programId, zone_type: zoneType, name: v.name || undefined, config: buildConfig(v, candidateId, matrixPairs) },
      {
        onSuccess: () => {
  ...
  const togglePair = (zoneId: string, cxuId: string) =>
    setMatrixPairs((prev) =>
      prev.some((p) => p.zoneId === zoneId && p.cxuId === cxuId)
        ? prev.filter((p) => !(p.zoneId === zoneId && p.cxuId === cxuId))
        : [...prev, { zoneId, cxuId }],
    );

  const configJson = JSON.stringify(
    { zone_type: zoneType || undefined, name: values.name || undefined, ...buildConfig(values, candidateId, matrixPairs) },
    null, 2,
  );
      ...
      >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="configure" className="space-y-3 mt-2">
            {/* Basic fields */}
            <div>
              <label className="text-sm font-medium" htmlFor="zn-name">Name</label>
              <Input id="zn-name" {...register('name')} placeholder="e.g. TME Zone A" />
            </div>
            <div>
              <label className="text-sm font-medium" htmlFor="zn-desc">Description</label>
              <Textarea id="zn-desc" rows={2} {...register('description')} placeholder="What this zone models…" />
            </div>
            <div>
            ...
            <div>
              <label className="text-sm font-medium">Asset / Candidate</label>
            ...
            <div className="rounded-md border">
              ...
              {advancedOpen && (
                <div className="border-t px-3 pb-3 pt-2 space-y-3">
                  ...
                  <div>
                    <label className="text-sm font-medium" htmlFor="zn-guardian">Guardian ID</label>
                    <Input id="zn-guardian" {...register('guardian_id')} placeholder="UUID or guardian name" />
                  </div>
                  <div>
                    <label className="text-sm font-medium" htmlFor="zn-drift">Drift Threshold (0–1)</label>
                    <Input
                      id="zn-drift" type="number" min={0} max={1} step={0.01}
          ...
          <TabsContent value="expert" className="space-y-4 mt-2">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <div>
                  <label className="text-sm font-medium" htmlFor="zn-esc">Escalation Threshold (0–1)</label>
                  <Input
                    id="zn-esc" type="number" min={0} max={1} step={0.01}
                    placeholder="e.g. 0.8"
        ...
        <DialogFooter className="gap-2 pt-2">
          ...
          <Button
            type="button"
            onClick={onRunNow}
            disabled={!zoneType || createZone.isPending}
          >
            {createZone.isPending ? 'Creating…' : 'Run Now →'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
